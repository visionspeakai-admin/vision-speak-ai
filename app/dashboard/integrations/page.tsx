...
import { Plus, CheckCircle2, Zap, Loader2, Link2, Settings2 } from 'lucide-react'
...
export default function IntegrationsPage() {
  ...
  return (
    <div className="space-y-8 animate-fade-in">
        ...
        <div className="animate-fade-up">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {connectedIntegrations.map((integration, index) => (
              <div key={index} className="glass-effect p-6 rounded-xl flex items-start justify-between border border-white/5 hover:border-cyan-400/20 transition-all">
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      <h3 className="font-bold text-white group-hover:text-cyan-electric transition-colors">{integration.name}</h3>
                      {integration.popular && (
        ...
        <DialogContent className="sm:max-w-[425px] glass-effect-strong border-white/10 bg-obsidian-dark text-white">
          <DialogHeader>
            ...
            <DialogDescription className="text-slate-400">
              Configure your {selectedIntegration?.name} integration to start receiving telemetry data.
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-6">
            <div className="grid gap-2">
              <Label htmlFor="config" className="text-[10px] font-black tracking-widest text-slate-500 uppercase">
              ...
              <p className="text-[10px] text-slate-600 mt-1">Visit your {selectedIntegration?.name} dashboard to generate this key.</p>
            </div>
          </div>
          <DialogFooter className="flex flex-col sm:flex-row gap-3">
            ...
            >
              {isConnecting ? <Loader2 className="animate-spin" size={16} /> : <CheckCircle2 size={16} />}
              Confirm Connection
            </button>
          </DialogFooter>
        </DialogContent>
      ...
      {/* Custom Integration */}
      <div className="glass-effect-strong border-cyan-400/50 shadow-[0_0_30px_rgba(0,242,255,0.3)] p-8 rounded-xl animate-fade-up" style={{ animationDelay: '200ms' }}>
        <div className="flex items-start gap-4">
          <Zap className="text-cyan-400 flex-shrink-0 mt-1" size={24} />
          <div>
            <h3 className="font-bold text-white mb-2 underline decoration-cyan-400/30 underline-offset-4">Need a Custom Integration?</h3>
            <p className="text-slate-400 mb-4 text-sm leading-relaxed">
              We support webhooks and REST APIs for custom integrations. Check our API documentation for details.
            </p>
            <a href="/developers" className="inline-flex items-center gap-2 text-cyan-400 font-bold hover:text-cyan-300 transition-all text-sm group">
              View Integration Guide
              <Link2 size={16} className="group-hover:translate-x-1 transition-transform" />
            </a>
          </div>
        </div>
      </div>
